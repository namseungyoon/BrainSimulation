...
| Ex2b | 연결 검증 매트릭스 (전 경로 2세포 uPSP/PPR) | `04_experiments/Ex2b_connection_matrix` | 🔄 커넥톰 UI(원형+매트릭스) ✅ · 132경로·5클래스 · 2세포 벤치 대기(Ex3 후) |
...
- **현재(05)**: 국소 SC 시냅스에 **직접** 구동력/자극 인가 (후시냅스 point 방식, 기존 논문 관행). E1~E11 즉시 진행.
- **미래**: **전시냅스 섬유 기반 SC 자극 모델**은 별도 트랙 **`06_Presynaptic_SC_Stimulation/`** 에서 재구현 → 완성 시 05에 소급 적용. (05 자산 `sc_synapses.npz`·`sc_fibers.npz` 참조)
