...
- 문헌 대조 성격은 실험마다 다름 — 타이밍은 파라미터를 피팅하지 않은 **Bi & Poo 1998 독립 대조**, 버스트 수는 기본값 출처인 **Wittenberg & Wang 2006 구현 검증**임. (세 실험은 tr8 완주 후 실행 예정 — 아래는 프로토콜 도해와 예상 모식도, 실제 산출값으로 교체)

##### 1. STDP 타이밍 곡선 — 부호·창 독립 대조 (진행 예정)

- 후시냅스 버스트를 고정한 채 전·후 스파이크 시간차 Δt를 음수에서 양수까지(−100에서 +100 ms) 여러 값으로 훑으며(sweep) 각 Δt에서 유도 후 rho 변화를 측정하여 STDP 곡선을 산출함. Δt가 양수로 전시냅스가 후시냅스보다 먼저 발화할 때 강화 LTP, 음수로 순서가 뒤바뀔 때 억압 LTD로 갈리는 부호와, 강화·억압이 나타나는 시간창의 폭이 재현 대상임. 유도 프로토콜은 전시냅스 단일 스파이크와 후시냅스 버스트를 Δt 간격으로 짝지어 5 Hz로 유도 반복 30회 인가하는 구성임
...
- (결과 진행 예정) 산출한 Δt 곡선을 ca_stp 0(원본)과 1(확장)에 대해 병렬로 제시하고, Bi와 Poo 1998의 STDP 창과 부호·형태를 대조할 예정임. 아래는 예상 곡선 모식도임

[그림 가-15. 예상 STDP 타이밍 곡선(모식도 · 실제 데이터 아님) — Δt 대 rho 변화, ca_stp 0·1 병렬, 양(+)Δt 강화·음(−)Δt 억압]  (`04_experiments/Ex10_STDP_pair/figures/stdp_curve_expected.png`)

##### 2. post-burst 수 의존 — Wittenberg 재현 (진행 예정)

- 시간차 Δt를 고정한 채 후시냅스 버스트의 발수를 1발·2발·4발로 바꿔 가며 rho 변화를 측정함. 단일 스파이크(1발)에서는 칼슘이 강화문턱에 못 미쳐 변화가 없거나 억압으로, 버스트 발수가 늘수록 칼슘이 문턱을 넘어 강화로 부호가 바뀌는 것이 재현 대상임. 본 모델 경로(CA3에서 CA1 추체세포)와 기본값(Wittenberg 2006 피팅치)이 이 실험의 대조 문헌과 동일하므로, 버스트 수가 STDP 부호를 가르는 Wittenberg의 핵심 관찰을 직접 재현하는지 확인함

[그림 가-16. 예상 post-burst 수 의존(모식도 · 실제 데이터 아님) — 후시냅스 1·2·4발에 따른 rho 변화, 단일=무변화/억압 → 버스트=강화]  (`04_experiments/Ex10_STDP_pair/figures/stdp_burstnum_expected.png`)

##### 3. pairing 주파수 의존 — Sjöström 재현 (옵션, 진행 예정)

- 유도 반복의 빈도를 저빈도에서 고빈도까지(1·5·20·50 Hz) 바꿔 가며 rho 변화를 측정함. 저빈도에서는 스파이크 순서에 따라 억압이 우세하고, 고빈도에서는 칼슘 트랜지언트가 누적되어 순서와 무관하게 강화로 전환되는 빈도 의존 전환이 재현 대상임. 대조 문헌 Sjöström et al. 2001은 신피질 자료여서 정량이 아닌 정성 대조에 해당하며, 본 실험은 코어 두 실험(타이밍·버스트 수)을 확정한 뒤 여력에 따라 진행하는 옵션임

[그림 가-17. 예상 pairing 주파수 의존(모식도 · 실제 데이터 아님) — 반복 빈도 1~50 Hz에 따른 rho 변화, 저빈도 억압 → 고빈도 강화 전환]  (`04_experiments/Ex10_STDP_pair/figures/stdp_freq_expected.png`)
