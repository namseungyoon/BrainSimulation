...
- **⭐ 억제=전압클램프(VC) IPSC**(08-26): 억제 시냅스는 E_GABA≈rest라 **rest에서 안 보임**·전류주입으론 저(低)입력저항 세포를 못 올림 → **SEClamp −50mV 고정 후 IPSC 측정**(실험실 표준). 흥분=전류클램프 EPSP. ⚠️ **수상돌기 억제(Ivy·OLM·SCA)는 소마 VC 공간클램프로 과소·저속 측정**(실제 실험 동일 한계) — 잠복 등 이차지표 부정확할 수 있음, PPR(비율)은 상대적으로 강건.
