# -*- coding: utf-8 -*-
"""
11_schaffer/e2c_gpu_perspectives2.py — 결정론 GPU 전 슬라이스 추가 다관점 분석.
perspectives2 로직 그대로(CPU판 e2c_full_perspectives2.py 이식), 입력만 GPU 결정론 런으로 교체.
sc_det_gpu/fullscale_n4/SC_spikes_all.csv + slice_cells.npz(층/mtype/nd/xyz) + pruned_connectivity.npz.

산출(figures/):
  E2cGPU_dynamics.png : (A)깊이(nd)x시간 발화밀도 (B)ISI-CV 분포(PC/INT) (C)집단 동기화(5ms bin rate+Fano) (D)내부 9경로 활성
  E2cGPU_spatial.png  : 세포별 발화율 공간지도(2 투영)
실행: <ca1sim python> 11_schaffer/e2c_gpu_perspectives2.py
"""
import os, csv
import numpy as np
import matplotlib
...
def main():
    ...
    fig.suptitle("결정론 GPU 전 슬라이스 추가 관점 — 17,647세포·1초·결정론 시냅스+SC (CoreNEURON/A6000)",
                 fontsize=13, fontweight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.savefig(os.path.join(FIG, "E2cGPU_dynamics.png"), dpi=130); plt.close(fig)
    ...
    fig2.suptitle("결정론 GPU 전 슬라이스 — 세포별 발화율 공간지도 (17,647세포)",
                  fontsize=13, fontweight="bold")
    fig2.tight_layout(rect=[0, 0, 1, 0.95])
    fig2.savefig(os.path.join(FIG, "E2cGPU_spatial.png"), dpi=130); plt.close(fig2)
    ...
    print(f"[그림] E2cGPU_dynamics.png · E2cGPU_spatial.png → {FIG}", flush=True)
