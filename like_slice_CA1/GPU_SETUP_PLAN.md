...
### GPU 재실행 목표 (성과 실험 — 검증·벤치 통과 후)
전부 **확률적 TM 시냅스**(BBP 확률적 AMPA/NMDA·GABA, 확률 방출)로 = 생물학적 충실. **전제: 확률 시냅스 GPU 검증(스모크→시냅스검증→벤치) 통과.**

| ID | 규모 | 입력 | 시간 | 시냅스 | CPU 기준 |
| --- | --- | --- | --- | --- | --- |
| **G1** | 17,647세포 | 배경 구동(baseline) | 1초 | 확률적 TM(내부) | CPU 실측 66.6h |
| **G2** | 2,000세포 + SC 경로 | SC 포아송 | 9초 | 확률적 TM(내부+SC) | CPU 54h(그땐 결정론 → GPU선 **확률로 변경**) |
| **G3** | 17,647세포 + SC 경로 | SC 구동 | 1초 | 확률적 TM(내부+SC) | CPU 불가(~64h/1초) |

검증 순서: ①GPU 스모크(수백 세포) → ②확률 시냅스 GPU 컴파일·실행 검증(관문, 실패 시 확률 미포함으로 폴백) → ③벤치(G2 규모 CPU 대비 배속) → ④G1·G2·G3. 검증은 GPU-CPU 통계 비교(비트-동일 아님).
