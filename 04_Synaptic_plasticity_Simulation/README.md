...
| **4-3** 부과 theta | ✅ **미결#5 해소 — 기본 = 정현파 전류.** 정현파는 **65.4 mV/nA 선형**(목표 오차 8.8%), 억제리듬은 **포화**(세기 256배 → 진폭 17.9배, 선형성 0.070) 하고 목표 8mV에 **도달 불가**(최대 6.10mV) — 막전위가 `e_GABAA`로 접근해 구동력이 사라진다 · ★**위상 기준 = 시냅스 국소 Vm** (주입→소마 **+86.7°**(5Hz)/+3.6°(8Hz)로 주파수마다 다르므로 주입 위상은 쓸 수 없다. 소마→시냅스는 ≤2.78°) (15/15) |
| **4-4** 부과 gamma | ✅ ★**주입 비 ≠ 막에서의 비** — 주입 0.35 → 막 0.135/0.098/0.074 (30/50/80Hz, 저역통과 2.6~**4.7배** 감쇠) · gamma는 theta보다 시냅스까지 덜 간다(80Hz 0.50 vs theta 0.71~0.79) · 시냅스 gamma 위상 최대 **+60.8°** 밀림 · ★PAC 부과 **0.905** vs 대조 **0.004**(막의 잔여 결합 무시 가능) · ★PAC 측정법 함정 — 잔차 RMS로 재면 대조도 0.41 (10/10) |
| **4-5** 위상 정렬 검증 | ✅ ★**측정 위상으로 조준하니 오차 0.0°** (주입 파형에 맞추면 5Hz에서 116° 어긋난다) · 주입→소마 지연 **−3.32°/+3.57°** — 4-3의 "+86.7°"는 제 규약 오독이었고 정정 · ★**burst는 위상의 한 점이 아니다** — 퍼짐 **54.0°**(5Hz)/**86.4°**(8Hz), 이론과 일치 · 소마↔시냅스 **2.27°** (문헌 변환 자유) (10/10) |
| **4-6** 런타임 예산 | ✅ ★**원안 1145시간(48일) — 불가능** · 그러나 **프로토콜을 바꾸지 않는 손잡이**만으로 **77.4시간(3.2일)**: 병렬 10프로세스 + 유도 후 관찰 제거(효능 시상수 688초라 20초는 안 움직인다) · ★구성별 속도는 **별도 프로세스**여야 한다(NEURON은 전역 모델 하나 — 같은 프로세스에서 재면 프로브가 벤치보다 느리게 나온다) · 벤치 195.7 s/s vs 프로브 0.166 s/s = **1181배** · 남는 축소 대상은 6-3·6-4·6-7 셋뿐 (11/11) |
| **6-1** theta 위상 가소성 (**첫 실험**) | ✅ ★**결핍 G6 확정 — 위상에 따라 부호가 갈리지 않는다**(문헌: peak LTP / trough LTD) · 원인 분리: GB 계열·STDP 는 칼슘이 **스파이크 이벤트**에서만 나와 **구조적으로 불가능**(폭 1e-4) · `glu` 만 위상을 본다(폭 4.1e-3, **방향은 문헌과 일치**) 그러나 4% 변조로 부호를 못 바꾼다 — 시냅스 theta 6.09mV vs bAP 95mV · ★**theta 를 키워도 안 된다** — 36.5mV 에서 theta 자체가 17~18발을 만들어 실험 무효 (11/11) |
| 6-2 ~ 6-9 · 7 | ⬜ — 6-1 이 7단계 요구사항을 하나 확정했다 |
